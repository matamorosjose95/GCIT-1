...
def training(dataset, training_rule, l_r= 1e-4,Iterations = 1000, batch_size = 64, lambda_x = 10,lambda_z = 1, gamma = 0.8, n_z = 62):
  # Models
  ...
  xMI_loss  = []
  # training data
  x_train, y_train, z_train = dataset[:]
  # Noise
  v_mb = torch.FloatTensor(batch_size, n_z).cuda()
  # data evolution
  fine_tunning = 5
  if training_rule == 'GCIT_InfoGAN':
    print('Entrenando Modelo propuesto por nosotr@s:')
    print('Minimización de la Información mutua entre G(z,v) y x')
    print('Maximización de la Información mutua entre G(z,v) y z')
    ...
    for i in range(Iterations):
      ...
      #  break
    pbar.close()
    #Losses_plot(G_loss,D_loss,xMI_loss,zMI_loss,'MB')
    return zMI_loss,xMI_loss, G_loss, D_loss
  # InfoGAN
  if training_rule == 'InfoGAN':
    print('Entrenando Modelo propuesto en InfoGAN')
    pbar = tqdm(total=Iterations)
    for i in range(Iterations):
      for k in range(fine_tunning):
        ...
        idx = np.random.permutation(len(x_train))
        zMI_optimizer.zero_grad()
        MINE_loss = -1*(torch.mean(zMI_net.forward(x_fk.detach(), z_mb)) - torch.log(torch.mean(torch.exp(zMI_net.forward(x_fk.detach(), z_hat_mb)))))
        MINE_loss.backward()
        zMI_optimizer.step()
        # end fine tunning
      D_loss.append(Discriminator_loss.detach().cpu().numpy().item())
      zMI_loss.append(-1*MINE_loss.detach().cpu().numpy().item())
      ### Entrenar RED GENERADORA
      idx = np.random.permutation(len(x_train))
      v_mb.uniform_(0,1).cuda()
      z_hat_mb, x_hat_mb, z_mb, x_mb = data_perm(x_train, z_train, batch_size)
      x_fk = G_net.forward(z_mb, v_mb)
      G_optimizer.zero_grad()
      INFO = torch.mean(zMI_net.forward(x_fk, z_mb).detach())
      Generator_loss = -1*(torch.mean(D_net.forward(x_fk.detach(),z_mb) - D_net.forward(x_mb, z_mb)) - lambda_z*INFO)
      Generator_loss.backward()
      G_optimizer.step()
      G_loss.append(-1*Generator_loss.detach().cpu().numpy().item())
      pbar.update(1)
    ...
    #Losses_plot(G_loss,D_loss,xMI_loss,zMI_loss, 'infogan')
    return zMI_loss, G_loss, D_loss
  if training_rule == 'Bellot':
    print('Entrenando Modelo propuesto en CIT using GANs')
    pbar = tqdm(total=Iterations)
    for i in range(Iterations):
      for k in range(fine_tunning):
        ### ENTRENAR RED DISCRIMINADORA
        v_mb.uniform_(0,1).cuda()
        z_hat_mb, x_hat_mb, z_mb, x_mb = data_perm(x_train, z_train, batch_size)
        x_fk = G_net.forward(z_mb, v_mb).detach()
        D_optimizer.zero_grad()
        Discriminator_loss = torch.mean(D_net.forward(x_mb, z_mb) + 1 - D_net.forward(x_fk, z_mb))
        Discriminator_loss.backward()
        D_optimizer.step()
        ### ENTRENAR MINE
        xMI_optimizer.zero_grad()
        MINE_loss = -1*(torch.mean(xMI_net.forward(x_fk.detach(), x_mb)) - torch.log(torch.mean(torch.exp(xMI_net.forward(x_fk.detach(), x_hat_mb)))))
        MINE_loss.backward()
        xMI_optimizer.step()
        # end fine tunning
      D_loss.append(Discriminator_loss.detach().cpu().numpy().item())
      xMI_loss.append(-1*MINE_loss.detach().cpu().numpy().item())
      ### Entrenar RED GENERADORA
      idx = np.random.permutation(len(x_train))
      v_mb.uniform_(0,1).cuda()
      z_hat_mb, x_hat_mb, z_mb, x_mb = data_perm(x_train, z_train, batch_size)
      x_fk = G_net.forward(z_mb, v_mb)
      G_optimizer.zero_grad()
      mine_loss = torch.mean(xMI_net.forward(x_fk, x_mb)) - torch.log(torch.mean(torch.exp(xMI_net.forward(x_fk, x_hat_mb))))
      Generator_loss = -1*(torch.mean(D_net.forward(x_fk.detach(),z_mb) - D_net.forward(x_mb, z_mb)) + lambda_x*mine_loss)
      Generator_loss.backward()
      G_optimizer.step()
      G_loss.append(-1*Generator_loss.detach().cpu().numpy().item())
      pbar.update(1)
    ...
    G_loss = np.array(G_loss)
    #Losses_plot(G_loss,D_loss,xMI_loss,zMI_loss,'bellot')
    return xMI_loss, G_loss, D_loss